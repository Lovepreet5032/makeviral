...
public class AddtoCampignAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ArrayList<TagDataObject.Email> mNotifcationList;
...
//int position=0;
public AddtoCampignAdapter(ArrayList<TagDataObject.Email> mNotifcationList, Context context,ArrayList<ContactManagerDataObject.ContactManagerListData> data,String strtype){//,ContactAdapterInterface listner) {
...
public void onBindViewHolder(RecyclerView.ViewHolder vho, final int pos) {
        if (vho instanceof AddtoCampignAdapter.ViewHolder) {
        AddtoCampignAdapter.ViewHolder vh = (AddtoCampignAdapter.ViewHolder) vho;
            TagDataObject.Email videoPos = mNotifcationList.get(pos);
        vh.bindVideo(videoPos,pos);
        }
        else if(vho instanceof AddtoCampignAdapter.EmptyViewHolder)
        {
        ((AddtoCampignAdapter.EmptyViewHolder)vho).textView.setText(context.getString(R.string.no_campign));
        }
        }
...
public class ViewHolder extends RecyclerView.ViewHolder {
    private TagDataObject.Email mVideoModels;
    ...
    public void bindVideo(TagDataObject.Email videoModel,final int position) {

        mVideoModels = videoModel;
        txt_campignname.setText(mVideoModels.getCampaignName());
        btn_action.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                addtoCoampign(mVideoModels);
            }
        });
    }
}
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        ...
        if (viewType == EMPTY_VIEW) {
            ...
            AddtoCampignAdapter.EmptyViewHolder evh = new AddtoCampignAdapter.EmptyViewHolder(v);
            return evh;
        }
        else {
            v = LayoutInflater.from(parent.getContext()).inflate(R.layout.row_addtocapmpign, parent, false);
            AddtoCampignAdapter.ViewHolder vh = new AddtoCampignAdapter.ViewHolder(v);
            return vh;
        }
    }
    private void addtoCoampign(final TagDataObject.Email mVideoModels) {
       progressDialog=new ProgressDialog(context);
        ...
        StringRequest stringRequest = new StringRequest(Request.Method.POST, ConfigURL.ADDPROTOCAMPAIGN, new Response.Listener<String>() {
            @Override
            ...
            protected Map<String, String> getParams() throws AuthFailureError {
                ...
                params.put("cmp_type",strtype);
                params.put("cmp_id",mVideoModels.getId());
                return params;
            }
        };
...
//    private void addToGroup(final boolean showLoading, final String camId, final String campName) {
//
//        progressDialog.setMessage("Loading");
//        if(showLoading){
//            if(progressDialog!=null&&!progressDialog.isShowing()){
